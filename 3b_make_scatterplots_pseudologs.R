...
for (file in c("average_signals/human_nopseudo_average_signal.txt",
	...
	for (sample in samples){
		if ((paste0(sample, "-", "PlusTag") %in% samples2) && (paste0(sample, "-", "MinusTag") %in% samples2)){
			...
			#- make a scatterplot 
			print(paste0("scatterplots/scatterplot_log_", sample, type, "_",species,".png"))
			png(paste0("scatterplots/scatterplot_log_", sample, type, "_",species,".png"))
			range_vals <- range(c(fileall$signal_m , fileall$signal_p))
			p <- ggplot(fileall, aes(x=signal_m, y=signal_p)) +
				geom_point() +
 				scale_x_continuous(trans = scales::pseudo_log_trans(base = 10), limits = range_vals) +
				scale_y_continuous(trans = scales::pseudo_log_trans(base = 10), limits = range_vals) +
				geom_abline(intercept = 0, slope = 1, linetype = "dotted", color = "red") +  # Add diagonal line
				ggtitle(sample) +
				xlab("MinusTag (pseudo-log scale)") +
				ylab("PlusTag (pseudo-log scale)") +
				theme_minimal() +
   				theme(
			...
			#- make a scatterplot comparison of normal and log
			print(paste0("scatterplots/scatterplot_comparison_", sample, type, "_",species,".png"))
			png(paste0("scatterplots/scatterplot_comparison_", sample, type, "_",species,".png"), width = 1000, height = 500)
			p_linear <- ggplot(fileall, aes(x = signal_m, y = signal_p)) +
				geom_point() +
				geom_abline(intercept = 0, slope = 1, linetype = "dotted", color = "red") +
				ggtitle(paste0 (sample, " - Linear Scale")) +
				xlab("MinusTag") +
				ylab("PlusTag") +
				theme_minimal()+
				xlim (range_vals) +
				...
				theme(
    			    ...
    			    axis.text = element_text(size = 16),   # Axis tick labels
    			    plot.title = element_text(size = 22, face = "bold"))  # Title size and weight	
			p_pseudolog <- ggplot(fileall, aes(x = signal_m, y = signal_p)) +
				geom_point() +
				scale_x_continuous(trans = scales::pseudo_log_trans(base = 10), limits = range_vals) +
				scale_y_continuous(trans = scales::pseudo_log_trans(base = 10), limits = range_vals) +
				geom_abline(intercept = 0, slope = 1, linetype = "dotted", color = "red") +
				ggtitle(paste0 (sample, " - Pseudo-Log Scale")) +
				xlab("MinusTag") +
				ylab("PlusTag") +
				theme_minimal()+
   				theme(
    			    ...
    			    axis.text = element_text(size = 16),   # Axis tick labels
    			    plot.title = element_text(size = 22, face = "bold"))  # Title size and weight	
			gridExtra::grid.arrange(p_linear, p_pseudolog, ncol = 2)
			dev.off()
		}
		...
		if ((str_split_fixed(sample, "_", n=2)[1] != "GFP") && 
			...
			#- make a scatterplot
			print(paste0("scatterplots_GFP/scatterplot_log_", sample, type, "_",species,".png"))
			png(paste0("scatterplots_GFP/scatterplot_log_", sample, type, "_",species,".png"))
			range_vals <- range(c(fileall$signal_m, fileall$signal_p))
			p <- ggplot(fileall, aes(x=signal_m, y=signal_p)) +
				geom_point() +
				scale_x_continuous(trans = scales::pseudo_log_trans(base = 10), limits = range_vals) +
			...
			#- make a scatterplot comparison of normal and log
			print(paste0("scatterplots_GFP/scatterplot_comparison_", sample, type, "_",species,".png"))
			png(paste0("scatterplots_GFP/scatterplot_comparison_", sample, type, "_",species,".png"), width = 1000, height = 500)
			p_linear <- ggplot(fileall, aes(x = signal_m, y = signal_p)) +
				geom_point() +
				geom_abline(intercept = 0, slope = 1, linetype = "dotted", color = "red") +
				ggtitle(paste0 (sample, " - Linear Scale")) +
